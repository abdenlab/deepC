...
1) Saliency mapping: > remove all tf.cast functions for salieny mapping (gradient of regresion score with respect to input)
...
def slice_seqs(
    positions, remainder, bp_context, batch_size, chrom_seq, translation_tensor
):
...
def dilated_layer(
    ...
    with tf.name_scope(name):
        ...
        if residual == True:
            if dense_residual == True:
                dense_weights = create_variable(
                    "dense_weights", [dilation_units, channels, dilation_units]
                )
        ...
        gated = tf.add(
            dilated_conv(input_batch, gate_weights, dilation=dilation), gate_biases
        )
...
def convolutional_layer(
    ...
    with tf.name_scope(name):
        ...
        # define convolutional steps
        conv = tf.add(
            tf.nn.conv1d(input_batch, weights, stride=1, padding="SAME"), biases
        )
...
def loss(regression_score, labels, l2_regularization_strength, batch_size):
    ...
    with tf.name_scope("Loss"):
        ...
        mean_squared_error = tf.losses.mean_squared_error(
            labels, regression_score, reduction=tf.losses.Reduction.SUM
        )
...
def loss_test(regression_score, labels, batch_size):
    ...
    with tf.name_scope("Test_Loss"):
        labels = tf.to_float(labels)
        test_mean_squared_error = tf.losses.mean_squared_error(
            labels, regression_score, reduction=tf.losses.Reduction.SUM
        )
...
def training(loss, learning_rate, beta_1, beta_2, epsilon, global_step):
    ...
    optimizer = tf.train.AdamOptimizer(
        learning_rate=learning_rate, beta1=beta_1, beta2=beta_2, epsilon=epsilon
    )
