...
flags.DEFINE_boolean(
    "mutate_sequence",
    False,
    "Seelect if the sequence specified should be mutated. If False will just extract the sequence specified as a full bed window. [Default  False]",
)
flags.DEFINE_string(
    "report_format", "long", "How to report the saliency scores ['long' or 'broad']"
)
flags.DEFINE_string(
    "report_summary",
    "abs_sum",
    "What to report in long format: 'sum' of all columns (bases); 'abs_sum' the absolute sum; or all 4 'columns' one per base (A,C,G,T)",
)
flags.DEFINE_boolean(
    "times_seq",
    True,
    "Multiply gradients/saliency with hot encoded sequence element wise.",
)
flags.DEFINE_boolean(
    "report_wig",
    False,
    "[True False] select if to also write a wiggle track. Currently only works for report_summary abs_sum or sum. If selected, bp_context must amtch the input sequence window (bed format).",
)
flags.DEFINE_string(
    "wig_name_tag", "sal", "Nametag for wiggle file if specified to report this way."
)
flags.DEFINE_string(
    "wig_description",
    "saliency per bp",
    "Description for wiggle track if specified o report.",
)
flags.DEFINE_boolean(
    "filter_zeros",
    True,
    "[True False] select if to filter out value zero entries (onlf for txt output).",
)
# Network Architechture
flags.DEFINE_boolean(
    "report_conv_hidden_state",
    False,
    "[True False] Select if only to report the hidden state after convolution.",
)
flags.DEFINE_integer("conv_layers", 6, "Number of convolutional layers.")
flags.DEFINE_string(
    "hidden_units_scheme",
    "300,600,600,900,900,100",
...
flags.DEFINE_string(
    ...
    "Comma seperated dilation scheme to use..",
)
...
def placeholder_inputs(batch_size, dtype):
    ...
    if dtype == "bool":
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.bool, [None, FLAGS.bp_context, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.uint8, shape=[None, FLAGS.num_classes], name="labels"
        )
    if dtype == "uint8":
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.uint8, [None, FLAGS.bp_context, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.uint8, shape=[None, FLAGS.num_classes], name="labels"
        )
    if dtype == "float32":
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.float32, [None, FLAGS.bp_context, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
        ...
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.int32, [None, FLAGS.bp_context, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.int32, shape=[None, FLAGS.num_classes], name="labels"
        )
        ...
        with open(FLAGS.input, "r") as rdf:
            ...
            for line in rdf:
                ...
                if FLAGS.mutate_sequence == True:
                    ...
                    if (
                        deletion_flag == 1
                    ):  # adjust for full deletions as the first base pair goes as well
                    ...
                    patch_start = (
                        customfloor(start, FLAGS.bin_size)
                        - (FLAGS.add_window)
                        - half_bp_context
                    )
                    patch_end = (
                        customceil(end, FLAGS.bin_size)
                        + (FLAGS.add_window)
                        + half_bp_context
                    )
                    patch_new_end = (
                ...
                while (
                    i
                    < (seq_length - FLAGS.bp_context + FLAGS.bin_size) / FLAGS.bin_size
                ):
                ...
                hotseqs = []
                for seq in run_seqs:
                    seq = get_hot_coded_seq(
                        seq, use_soft=FLAGS.use_softmasked
                    )  # hot encode
                    hotseqs.append(seq)
                ...
                outfile_name = (
                    FLAGS.out_dir
                    + "/"
                    + "saliencies_%s_%s_%s_%s_%s.txt"
                    % (FLAGS.name_tag, region_counter, chrom, start, end)
                )
                    ...
                    if FLAGS.report_format == "broad":
                        ...
                        for i in range(len(run_starts)):
                            ...
                            if FLAGS.report_summary == "columns":
                                ...
                                print(
                                    "No valid report mode specfied for long format! Select: ['abs_sum', 'sum', 'columns']"
                                )
                    ...
                    if FLAGS.report_wig == True:
                        if FLAGS.report_summary in ["abs_sum", "sum"]:
                            outwig_name = (
                                FLAGS.out_dir
                                + "/"
                                + "saliencies_%s_%s_%s_%s_%s.wig"
                                % (FLAGS.name_tag, region_counter, chrom, start, end)
                            )
                            wiggle_name = "saliency_%s_%s_%s" % (
                                FLAGS.report_summary,
                                FLAGS.wig_name_tag,
                                region_counter,
                            )
                            wiggle_description = "%s %s %s" % (
                                FLAGS.wig_description,
                                FLAGS.report_summary,
                                region_counter,
                            )
                            with open(outwig_name, "w") as wig:
                                ...
                                wig.write(
                                    "fixedStep chrom=%s start=%s step=1 span=1\n"
                                    % (chrom, start + 1)
                                )
                                ...
                                for i in range(len(run_starts)):
                                    ...
                                    if FLAGS.report_summary == "sum":
                                        ...
                                        print(
                                            "No valid report modefor wig reporting specfied! Select: ['abs_sum', 'sum']"
                                        )
