...
def batch_to_time(value, dilation, name=None):
    with tf.name_scope("batch_to_time"):
        ...
        return tf.reshape(
            transposed, [tf.math.divide(shape[0], dilation), -1, shape[2]]
        )
...
def convolutional_layer(
    ...
    with tf.name_scope(name):
        ...
        # Max Pool
        conv = tf.compat.v1.layers.max_pooling1d(
            conv,
            pool_width,
            strides=pool_width,
            padding="same",
...
def loss(regression_score, labels, l2_regularization_strength, batch_size):
    ...
    with tf.name_scope("Loss"):
        ...
        mean_squared_error = tf.compat.v1.losses.mean_squared_error(
            labels, regression_score, reduction=tf.compat.v1.losses.Reduction.SUM
        )
...
def loss_test(regression_score, labels, batch_size):
    ...
    with tf.name_scope("Test_Loss"):
        ...
        labels = tf.cast(labels, dtype="float32")
        test_mean_squared_error = tf.compat.v1.losses.mean_squared_error(
            labels, regression_score, reduction=tf.compat.v1.losses.Reduction.SUM
        )
