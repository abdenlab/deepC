...
flags.DEFINE_string(
    "data_file",
    "",
    "Input data: pseudo bed format: chr start end and comma separated classes",
)

# TRAININGS SETTINGS
flags.DEFINE_string(
    "test_chroms", "chr12, chr13", "Comma seperated list of test chromosoems to use ..."
)
flags.DEFINE_string(
    "validation_chroms",
    "chr16, chr17",
    "Comma seperated list of validation chromosoems to use ...",
)
...
flags.DEFINE_integer(
    "max_epoch", 2, "Number of epoch through train data to run trainer."
)
flags.DEFINE_integer(
    "max_chroms", 18, "Max number of training chromosomes to run through."
)
...
flags.DEFINE_float(
    "keep_prob_outer",
    0.8,
    "Keep probability for dropout. LEGACY Option not used in current model implementation.",
)
flags.DEFINE_integer("batch_size", 1, "Batch size.")
flags.DEFINE_float("l2_strength", 0.0001, "L2 regularization strength.")
flags.DEFINE_boolean(
    "shuffle", True, "If to shuffle the trainset at the start of each epoch."
)
# ARCHITECHTURE
...
flags.DEFINE_string(
    "hidden_units_scheme",
    "300,600,900,20",
    "Comma seperated hidden units scheme. Must have length of number of conv layers specified!",
)
flags.DEFINE_string(
    "kernel_width_scheme",
    "20,8,8,1",
    "Comma seperated kernel width scheme. Must have length of number of conv layers specified!",
)
flags.DEFINE_string(
    "max_pool_scheme",
    "5,5,5,1",
    "Comma seperated max pool scheme. Must have length of number of conv layers specified!",
)
# DILATIONAL
flags.DEFINE_string(
    "dilation_scheme", "2,4,8", "Comma seperated dilation scheme to use.."
)
flags.DEFINE_integer("dilation_units", 20, "Dilation Units (Filter).")
flags.DEFINE_integer(
    "dilation_width", 3, "Dilation Width (Only 2 supported at the moment)."
)
flags.DEFINE_boolean(
    "dilation_batch_norm",
    False,
    "If to apply batchnorm propagate residuals through the dilated layer stacks.",
)
# # RESIDUAL AND SKIP CONNECTIONS
flags.DEFINE_boolean(
    "dilation_residual",
    False,
    "If to propagate residuals through the dilated layer stacks.",
)
flags.DEFINE_boolean(
    "dilation_residual_dense",
    False,
    "If the residual/dilated layer should have a dense 1x1 convolution build in.",
)
...
# TRAIN LOCATION
flags.DEFINE_string(
    "train_dir", "training_run_data", "Directory to put the training data."
)
flags.DEFINE_string(
    "reload_model",
    "False",
    'If to reload a checkpoint/model file use string: "False" if not to reload (default); "continue" if to continue a training process on the same data or "transfer" if to load the entire model but strat training from scratch.',
)
...
flags.DEFINE_boolean(
    "seed_weights",
    False,
    "Select if to pre seed weights with numpy array stored weights. Convolutional only ...",
)
flags.DEFINE_string(
    "seed_scheme",
    "0,0,0",
    "Specify which layers are preseeded with the weights provided. [format: 1,1,0]",
)
flags.DEFINE_string(
    "seed_file",
    None,
    "Path to saved numpy file with saved weights. Weight and bias dimensions must match with the ones specified as hyper params for this run!",
)
# machine options
flags.DEFINE_integer(
...
flags.DEFINE_integer(
    "report_every",
    100,
    "Set interval of batch steps o when to report raining loss and log progress, losses and weights etc.",
)
# flag if to train with boolean values stored (labels and sequence)
flags.DEFINE_string(
    "store_dtype",
    "bool",
    "Indicate that sequence where stored as bools rather then integers. Will convert automatically.",
)
# Whole genome fasta file for accessing genome sequences
flags.DEFINE_string(
    "whg_fasta",
    None,
    "Path to whole genome fasta file for reading the genomic sequence",
)
flags.DEFINE_integer("seed", 1234, "Random seed for tensorflow (graph level).")

# GLOBAL Options ---------------------------------------------------------------
flags.DEFINE_integer(
    "bp_context", 1000000, "Number of classes to classify. Default 600."
)
flags.DEFINE_boolean(
    "use_softmasked",
    False,
...
if len(hidden_units_scheme) != FLAGS.conv_layers:
    print(
        "Hidden Units Scheme does not have the number of entries expected from 'conv_layers' ..."
    )
    sys.exit()
if len(kernel_width_scheme) != FLAGS.conv_layers:
    print(
        "Hidden Width Scheme does not have the number of entries expected from 'conv_layers' ..."
    )
    sys.exit()
if len(max_pool_scheme) != FLAGS.conv_layers:
    print(
        "Max Pool Scheme does not have the number of entries expected from 'conv_layers' ..."
    )
    sys.exit()
if FLAGS.seed_weights and len(seed_scheme) != FLAGS.conv_layers:
    print(
        "Seed Scheme does not have the number of entries expected from 'conv_layers' ..."
    )
    sys.exit()
...
def placeholder_inputs(batch_size, dtype):
    ...
    if dtype == "bool":
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.bool, [None, BP_CONTEXT, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.uint8, shape=[None, NUM_CLASSES], name="labels"
        )
    if dtype == "uint8":
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.uint8, [None, BP_CONTEXT, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.uint8, shape=[None, NUM_CLASSES], name="labels"
        )
        ...
        seqs_placeholder = tf.compat.v1.placeholder(
            tf.int32, [None, BP_CONTEXT, 4], name="seqs"
        )
        labels_placeholder = tf.compat.v1.placeholder(
            tf.int32, shape=[None, NUM_CLASSES], name="labels"
        )
...
def read_train_file(
    fi, num_classes, test_chromosomes, validation_chromosomes, dtype="int32"
):
...
def fill_seqs(
    chr_seq, positions_stored, indeces, batch_size, seq_length, dtype="uint8"
):
    # init
    if dtype == "bool":
    ...
    for i in range(batch_size):
        feed_seqs[i,] = chr_seq[
            positions_stored[indeces[i], 0] : positions_stored[indeces[i], 1]
        ]
...
def make_numpy_seed(
    seed_weights_list_loaded, seed_scheme, hidden_units_scheme, kernel_width_scheme
):
    ...
    for i in range(
        seed_sum
    ):  # TODO get number of layers to preload from preloading scheme
        w = i * 2
        b = w + 1
        ...
        if excess_neurons_in < 0 and excess_neurons_out and excess_neurons_width < 0:
            print(
                "Specified Convolutional Dimensions are smaller then what you are trying to preseed with. Only equal or larger allowed ..."
            )
            sys.exit()

        if excess_neurons_width > 0 or excess_neurons_in > 0 or excess_neurons_out > 0:
            print(
                "New specified dimensions in layer %s are larger then seed provided. Sampling excess weights from that distribution per layer!"
                % (i + 1)
            )
            excess_layer_count += 1
        # seed with whats available
        init_weights[
            0 : seed_weights_shape[0],
            0 : seed_weights_shape[1],
            0 : seed_weights_shape[2],
        ] = seed_weights
        ...
        if excess_neurons_out > 0:  # A) excess in hidden units dimension
            ...
            sampled = np.random.choice(
                seed_weights.flatten(), size=to_sample, replace=True
            )
            # fill array
        ...
        if (
            excess_neurons_in > 0
        ):  # B) excess in hidden units previous dimension (or input)
            ...
            # fill array
            init_weights[
                0:kw, seed_weights_shape[1] : indim, 0 : seed_weights_shape[2]
            ] = np.reshape(sampled, (kw, excess_neurons_in, seed_weights_shape[2]))

        if (
            excess_neurons_width > 0
        ):  # B) excess in hidden units previous dimension (or input)
            to_sample = (
                excess_neurons_width * seed_weights_shape[1] * seed_weights_shape[2]
            )
            sampled = np.random.choice(
                seed_weights.flatten(), size=to_sample, replace=True
            )
        ...
        if excess_neurons_out > 0:
            ...
            sampled = np.random.choice(
                seed_biases.flatten(), size=to_sample, replace=True
            )
...
def do_eval(
    ...
    for current_chr in test_chroms:
        ...
        while (sub_step + 1) * FLAGS.batch_size < sub_test_cases:
            ...
            feed_dict = {
                seqs_placeholder: fill_seqs(
                    tmp_chr_seq,
                    positions_test,
                    test_batch_range,
                    FLAGS.batch_size,
                    BP_CONTEXT,
                    dtype=FLAGS.store_dtype,
                ),
    ...
    print("Num examples: %d Test Loss: %0.04f" % (cases, test_loss))
...
def run_training():
    ...
    if FLAGS.seed_weights:
        ...
        seed_weights_list, excess_count = make_numpy_seed(
            seed_weights_list, seed_scheme, hidden_units_scheme, kernel_width_scheme
        )
    else:
        seed_weights_list = ""
    ...
    param_log_file = open(
        FLAGS.train_dir + "/hyperparameters_" + timestamp + ".log", "w"
    )
    param_log_file.write(
        "# Hyperparameters for dilationDHS run at: " + str(current_time)
    )
    ...
    param_log_file.write(
        "\nDropout Keep Probability Outer: "
        + str(FLAGS.keep_prob_outer)
        + " # LEGACY option not used in final models published."
    )
    ...
    param_log_file.write(
        "\nShuffle Training Set for each CHromosome Call: "
        + str(FLAGS.shuffle)
        + "\n\n"
    )
    ...
    with tf.Graph().as_default():
        ...
        seqs_placeholder, labels_placeholder = placeholder_inputs(
            FLAGS.batch_size, FLAGS.store_dtype
        )
        keep_prob_inner_placeholder = tf.compat.v1.placeholder(
            tf.float32, name="keep_prob_inner"
        )
        keep_prob_outer_placeholder = tf.compat.v1.placeholder(
            tf.float32, name="keep_prob_outer"
        )
        ...
        loss = deepCregr.loss(
            regression_score, labels_placeholder, FLAGS.l2_strength, FLAGS.batch_size
        )
        loss_test = deepCregr.loss_test(
            regression_score, labels_placeholder, FLAGS.batch_size
        )
        ...
        while epoch < FLAGS.max_epoch:
            ...
            for current_chr in to_train_chroms:
                ...
                # preload the respective chromosome sequence
                tmp_chr_seq = get_chr_seq(
                    FLAGS.whg_fasta, current_chr, FLAGS.store_dtype
                )
                ...
                while ((sub_step + 1) * FLAGS.batch_size) < sub_training_cases:
                    ...
                    _, loss_value, regr = sess.run(
                        [train_op, loss, regression_score], feed_dict=feed_dict
                    )
                ...
                if chrom_counter % FLAGS.save_every_chrom == 0:
                    ...
                    if best_loss > test_loss:
                        checkpoint_file = os.path.join(
                            FLAGS.train_dir, "best_checkpoint"
                        )
